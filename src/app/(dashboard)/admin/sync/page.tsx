...
export default function SyncPage() {
  ...
  const config = {
    "pg-to-maria": {
      label: "FacturApp → VB6",
      description:
        "Copie les données saisies dans FacturApp vers MariaDB pour que VB6 les voie",
      source: "PostgreSQL (FacturApp)",
      cible: "MariaDB (VB6)",
      couleur: "indigo",
      warning:
        "Les données MariaDB/VB6 seront écrasées par celles de FacturApp.",
      useCasePrimary: true,
    },
    "maria-to-pg": {
      label: "VB6 → FacturApp",
      description:
        "Copie les données saisies dans VB6 vers PostgreSQL pour les retrouver dans FacturApp",
      source: "MariaDB (VB6)",
      cible: "PostgreSQL (FacturApp)",
      couleur: "amber",
      warning:
        "Les données FacturApp/PostgreSQL seront écrasées par celles de VB6.",
  ...
  return (
    <div className="p-4 md:p-6 max-w-3xl">
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        {(
          Object.entries(config) as [Direction, (typeof config)[Direction]][]
        ).map(([key, cfg]) => (
          <button
            key={key}
            type="button"
          ...
          >
            ...
            <div className="flex items-center gap-1.5 text-xs text-slate-500 mb-2">
              <span
                className={`font-medium ${direction === key ? (key === "pg-to-maria" ? "text-indigo-600" : "text-amber-600") : ""}`}
              >
                {cfg.source}
              </span>
              <svg
      ...
      {/* Avertissement */}
      <div
        className={`card p-3 mb-4 border ${
      ...
      {loading && (
        <div className="mt-3">
          ...
          <div className="w-full h-2.5 bg-slate-200 rounded-full overflow-hidden">
            <div
              className={`h-full rounded-full sync-progress ${
                direction === "pg-to-maria" ? "bg-indigo-500" : "bg-amber-500"
              }`}
            />
          </div>

          <p className="text-xs text-slate-400 mt-1.5">
            Ne fermez pas cette page jusqu&apos;à la fin de la synchronisation.
          </p>
        </div>
      )}
      ...
      {result && (
        <div
          className={`card overflow-hidden mt-4 ${result.success ? "border-green-200" : "border-red-200"}`}
        >
          <div
            className={`px-4 py-3 flex items-center justify-between ${result.success ? "bg-green-50" : "bg-red-50"}`}
          >
            <span
