    ...
    <div className="p-4 md:p-6 max-w-5xl">
      ...
      <div className="flex items-start justify-between mb-6">
            ...
            {estValidee &&
              (estPayee ? (
                <span className="badge badge-success">
                  Payée le{" "}
                  {new Date(facture.paiement!.datePaiement!).toLocaleDateString(
                    "fr-FR",
                  )}
                </span>
              ) : (
        ...
        <div className="flex gap-2 flex-wrap justify-end">
          ...
          <Link href="/paiements" className="btn-secondary btn-sm">
            Paiements
          </Link>
          <FactureActions facture={factureData} entreprise={entrepriseData} />
          {estValidee && !facture.aUnAvoir && (
            <AvoirButton factureId={facture.id} />
          )}
          {estValidee && !estPayee && (
            <DevaliderButton factureId={facture.id} />
          )}
          {facture.aUnAvoir && (
            <span className="badge badge-warning">Avoir créé</span>
          )}
        </div>
      </div>
          ...
          <div className="space-y-1 text-sm">
            ...
            <div className="flex justify-between text-base border-t border-indigo-200 pt-1 mt-1">
              ...
              <span className="font-bold text-indigo-700">
                {formatMontant(Number(facture.totalTtc))} MAD
              </span>
            </div>
            <div className="flex justify-between text-xs border-t border-indigo-100 pt-1 mt-1">
              ...
              <span className="text-emerald-600 font-medium">
                {formatMontant(Number(facture.margeHt))}
              </span>
            </div>
          </div>
        </div>
          ...
          <table className="data-table">
              ...
              {facture.lignes.map((l) => (
                <tr key={l.id}>
                  ...
                  <td className="font-semibold text-indigo-600">
                    {formatMontant(Number(l.montantTtc))}
                  </td>
                </tr>
              ))}
            </tbody>
            <tfoot className="bg-slate-50">
              <tr>
                <td
                  colSpan={6}
                  className="px-4 py-2 text-right text-sm font-medium text-slate-500"
                >
                  Totaux
                </td>
