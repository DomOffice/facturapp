...
import RechercheArticleFacture from "./recherche-article-facture";
    ...
    },
    {
      ...
      render: (row) => (
        <Link
          ...
          className="text-indigo-600 font-mono font-semibold hover:underline"
    ...
    },
    {
      ...
      render: (row) => (
        <span className="text-slate-500">
          {new Date(String(row.dateFacture)).toLocaleDateString("fr-FR")}
        </span>
      ),
    },
    {
      ...
      label: "Total HT",
      sortable: true,
      render: (row) => formatMontant(Number(row.totalHt)),
    },
    {
      key: "totalTtc",
      label: "Total TTC",
      ...
      render: (row) => (
        <span className="font-semibold text-indigo-700">
          {formatMontant(Number(row.totalTtc))}
        </span>
      ),
    },
    {
      ...
      <div className="page-header">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-0.5">
            {filtered.length} / {factures.length} facture(s)
          </p>
        </div>
      ...
      <div className="card overflow-hidden">
        <SortableTable
          ...
          mobileCard={(row) => (
            <Link
              href={`/factures/${row.id}`}
              className="card p-4 flex items-center justify-between active:bg-slate-50 block"
            >
              <div className="flex items-center justify-between w-full">
                <div>
                  ...
                  <div className="text-sm font-medium text-slate-700 truncate max-w-48 mt-0.5">
                    {String(row.clientNom)}
                  </div>
                  <div className="text-xs text-slate-400 mt-0.5">
                    {new Date(String(row.dateFacture)).toLocaleDateString(
                      "fr-FR",
                    )}
                  </div>
                </div>
