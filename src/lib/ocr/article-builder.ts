...
function calculerConfianceLigne(ligne: LigneFactureExtraite): number {
  ...
  if (
    ...
    let attendu = ligne.quantite * ligne.prixUnitaireTtc;

    if (ligne.remisePourcentage !== undefined && ligne.remisePourcentage > 0) {
...
function estDansColonne(
  ...
  return mot.x >= colonne.xMin && mot.x < colonne.xMax;
}
...
function detecterDonneesArticle(
  ...
  const reference = colonnes.reference
    ? tousLesMots.find(
        (mot) =>
          ...
          /^[A-Z0-9][A-Z0-9._/-]{2,}$/i.test(mot.texte.trim()),
      )
    : undefined;

  const textesDesignation = tousLesMots
    .filter((mot) => estDansColonne(mot, colonnes.designation))
  ...
  const tva = colonnes.tva
    ? tousLesMots.find(
        (mot) =>
          ...
          /^\s*(20|14|10|7|0)\s*%?\s*$/.test(mot.texte),
      )
    : undefined;
  ...
  const remise = colonnes.remise
    ? tousLesMots.find(
        (mot) =>
          estDansColonne(mot, colonnes.remise) && estPourcentage(mot.texte),
      )
    : undefined;

  const pu = tousLesMots.find(
    (mot) => estDansColonne(mot, colonnes.puTtc) && estMontant(mot.texte),
  );

  const qte = tousLesMots.find(
    (mot) => estDansColonne(mot, colonnes.quantite) && estEntier(mot.texte),
  );

  const total = tousLesMots.find(
    (mot) => estDansColonne(mot, colonnes.totalTtc) && estMontant(mot.texte),
  );
...
export function construireLigneArticleDepuisGroupes(
  ...
  const tauxTvaMatch = donnees.tva?.texte.match(/^\s*(20|14|10|7|0)\s*%?\s*$/);

  const referenceSeparee = donnees.reference?.texte.trim();
  ...
  const refEtDesignation = referenceSeparee
    ...
    : extraireReferenceDepuisDesignation(donnees.designationBrute);
  ...
  if (!ligne.reference) {
    ligne.alertes.push("Référence non détectée");
  }

  if (ligne.quantite === undefined) {
    ligne.alertes.push("Quantité non détectée");
  }

  if (ligne.prixUnitaireTtc === undefined) {
    ligne.alertes.push("Prix unitaire non détecté");
  }

  if (ligne.totalTtc === undefined) {
    ligne.alertes.push("Total ligne non détecté");
  }

  if (profil.colonnes.tva && ligne.tauxTva === undefined) {
    ligne.alertes.push("TVA non détectée");
  }
