...
async function main() {
  ...
  try {
    ...
    for (const c of clients) {
      const typeClientId = c.type_client ? (typeClientMap[c.type_client] ?? null) : null
      await pgClient.query(
        `INSERT INTO clients
         ...
         ON CONFLICT (id) DO UPDATE SET
           ...
           updated_at = NOW()`,
        [c.id, typeClientId, toNull(c.nom), toNull(c.adresse), toNull(c.code_postal),
         toNull(c.ville), toNull(c.telephone), toNull(c.ice), toNull(c.email)]
      )
    }
    await pgClient.query(`SELECT setval(pg_get_serial_sequence('clients','id'), COALESCE(MAX(id),1)) FROM clients`)
      ...
      await pgClient.query(
        `INSERT INTO fournisseurs
         ...
         ON CONFLICT (id) DO UPDATE SET
           ...
           email = EXCLUDED.email,
           updated_at = NOW()`,
        [f.id, toNull(f.nom), toNull(f.adresse), toNull(f.code_postal),
         toNull(f.ville), toNull(f.telephone), toNull(f.ice), toNull(f.email)]
      )
    }
    await pgClient.query(`SELECT setval(pg_get_serial_sequence('fournisseurs','id'), COALESCE(MAX(id),1)) FROM fournisseurs`)
      ...
      await pgClient.query(
        `INSERT INTO produits
         ...
         ON CONFLICT (id) DO UPDATE SET
           ...
           updated_at = NOW()`,
        [
          p.id, toNull(p.reference), toNull(p.article),
          p.fournisseur_id ? Number(p.fournisseur_id) : null,
          uniteId, tauxTvaId,
          prixAchatHt, prixVenteHt, margeHt,
          round2(prixAchatHt * (1 + tauxNum)),
          round2(prixVenteHt * (1 + tauxNum)),
        ]
      )
    }
    await pgClient.query(`SELECT setval(pg_get_serial_sequence('produits','id'), COALESCE(MAX(id),1)) FROM produits`)
    ...
    for (const f of factures) {
      const annee = toDate(f.date_facture)?.getFullYear() ?? new Date().getFullYear()
      ...
      await pgClient.query(
        `INSERT INTO factures
         ...
         ON CONFLICT (id) DO UPDATE SET
           ...
           updated_at = NOW()`,
        [f.id, numeroFormate, annee, seq, Number(f.client_id),
         toDate(f.date_facture), totalHt, totalTva, totalTtc]
      )
    }
    await pgClient.query(`SELECT setval(pg_get_serial_sequence('factures','id'), COALESCE(MAX(id),1)) FROM factures`)
    ...
    for (const fid of facIds) {
      await pgClient.query('DELETE FROM facture_lignes WHERE facture_id = $1', [fid])
    }
    ...
    for (const d of details) {
      ...
      await pgClient.query(
        ...
        [fid, d.produit_id ? Number(d.produit_id) : null, ordreLigne[fid],
         toNull(d.designation), qte, pu, remisePct, remiseHt,
         tauxTva, montantHt, montantTva, montantTtc]
      )
    }
    ...
    for (const p of paiements) {
      ...
      if (p.moyen_paiement) {
        const modeNorm = String(p.moyen_paiement).toLowerCase()
          .normalize('NFD').replace(/[\u0300-\u036f]/g, '')
        modeId = Object.entries(modeMap).find(([k]) => modeNorm.includes(k))?.[1] ?? null
      }
      await pgClient.query(
        ...
        [p.id, Number(p.facture_id), round2(toNum(p.montant_HT)), round2(toNum(p.Montant_TTC)),
         toDate(p.date_paiement), modeId, toNull(p.Numero_Piece), toNull(p.remarques)]
      )
    }
    await pgClient.query(`SELECT setval(pg_get_serial_sequence('paiements','id'), COALESCE(MAX(id),1)) FROM paiements`)
    ...
    try {
      ...
      for (const d of devis) {
        const annee = toDate(d.date_devis)?.getFullYear() ?? new Date().getFullYear()
        ...
        await pgClient.query(
          `INSERT INTO devis
           ...
           ON CONFLICT (id) DO UPDATE SET
             ...
             total_ttc = EXCLUDED.total_ttc,
             updated_at = NOW()`,
          [d.id, numeroFormate, annee, seq, Number(d.client_id),
           toDate(d.date_devis),
      ...
      await pgClient.query(`SELECT setval(pg_get_serial_sequence('devis','id'), COALESCE(MAX(id),1)) FROM devis`)
      ...
      for (const did of devisIds) {
        await pgClient.query('DELETE FROM devis_lignes WHERE devis_id = $1', [did])
      }
  ...
  [did, d.produit_id ? Number(d.produit_id) : null, ordreDevis[did],
   toNull(d.designation), round2(toNum(d.quantite)), round2(toNum(d.prix_unitaire)),
   round2(toNum(d.remise_pct) * 100), round2(toNum(d.remise_ht)),
   round2(toNum(d.taux_tva) * 100), montantHt, montantTva, round2(montantHt + montantTva)]
        )
      }
    ...
    try {
      ...
      for (const a of avoirs) {
        const annee = toDate(a.date_avoir)?.getFullYear() ?? new Date().getFullYear()
      ...
      await pgClient.query(`SELECT setval(pg_get_serial_sequence('avoirs','id'), COALESCE(MAX(id),1)) FROM avoirs`)
      ...
      const avoirIds = Array.from(new Set(avoirDetails.map(a => a.avoir_id)))
      for (const aid of avoirIds) {
        await pgClient.query('DELETE FROM avoir_lignes WHERE avoir_id = $1', [aid])
      }
      const ordreAvoir: Record<number, number> = {}
      for (const a of avoirDetails) {
        const aid = Number(a.avoir_id)
        ...
        await pgClient.query(
          ...
          [aid, a.produit_id ? Number(a.produit_id) : null,
           toNull(a.designation), round2(toNum(a.quantite)), round2(toNum(a.prix_unitaire)),
           round2(toNum(a.remise_pct) * 100), round2(toNum(a.taux_tva) * 100),
           montantHt, montantTva, round2(montantHt + montantTva)]
        )
      }
      console.log(`  ✅ ${avoirs.length} avoirs, ${avoirDetails.length} lignes`)
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : String(err)
      if (msg.includes("doesn't exist") || msg.includes('ER_NO_SUCH_TABLE')) {
