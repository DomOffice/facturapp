...
function coeffMarge(
  ...
  if (margeHtPg !== undefined && Number.isFinite(margeHtPg))
    return round2(margeHtPg);
  return 0;
}

// Ignore gracieusement les tables absentes dans MariaDB
async function syncOptional(
  tableName: string,
  fn: () => Promise<void>,
): Promise<void> {
  try {
...
async function main(): Promise<void> {
  ...
  try {
    ...
    await syncOptional("devis_details", () =>
      syncDevisLignes(pg, mysqlConn, lookups),
    );
    await syncOptional("avoirs", () => syncAvoirs(pg, mysqlConn));
    await syncOptional("avoir_details", () =>
      syncAvoirLignes(pg, mysqlConn, lookups),
    );
...
async function buildLookups(pg: Pool): Promise<LookupMaps> {
  ...
  for (const r of rows) {
    ...
    else if (typeCode === "type_fournisseur")
      typeFournisseurById.set(id, libelle);
  ...
  for (const r of rows) {
    ...
    await db.execute(
      ...
      [
        Number(r.id),
        r.type_client_id
          ? (lookups.typeClientById.get(Number(r.type_client_id)) ?? null)
          : null,
        toNullableString(r.raison_sociale),
        toNullableString(r.adresse),
        toNullableString(r.code_postal),
...
async function syncFournisseurs(
  pg: Pool,
  db: mysql.Connection,
  lookups: LookupMaps,
): Promise<void> {
  ...
  for (const r of rows) {
    ...
    await db.execute(
      ...
      [
        Number(r.id),
        r.type_fournisseur_id
          ? (lookups.typeFournisseurById.get(Number(r.type_fournisseur_id)) ??
            null)
          : null,
        toNullableString(r.raison_sociale),
        toNullableString(r.adresse),
  ...
  for (const r of rows) {
    ...
    await db.execute(
      ...
      [
        ...
        prixVenteHt,
        r.taux_tva_id
          ? (lookups.tauxTvaById.get(Number(r.taux_tva_id)) ?? 0.2)
          : 0.2,
...
async function syncFactureLignes(
  pg: Pool,
  db: mysql.Connection,
  lookups: LookupMaps,
): Promise<void> {
  console.log("• Sync facture_details");
  const { rows: factures } = await pg.query(
    `SELECT id FROM public.factures ORDER BY id`,
  );
  ...
  for (const f of factures) {
    ...
    await db.execute(`DELETE FROM facture_details WHERE facture_id = ?`, [
      factureId,
    ]);
    ...
    for (const r of rows) {
      ...
      if (r.produit_id) {
        const { rows: prod } = await pg.query(
          `SELECT unite_id FROM public.produits WHERE id = $1`,
          [Number(r.produit_id)],
        );
        if (prod[0]?.unite_id)
          unite = lookups.uniteById.get(Number(prod[0].unite_id)) ?? null;
      }
...
async function syncDevisLignes(
  pg: Pool,
  db: mysql.Connection,
  lookups: LookupMaps,
): Promise<void> {
  console.log("• Sync devis_details");
  const { rows: devis } = await pg.query(
    `SELECT id FROM public.devis ORDER BY id`,
  );
...
async function syncAvoirLignes(
  pg: Pool,
  db: mysql.Connection,
  lookups: LookupMaps,
): Promise<void> {
  console.log("• Sync avoir_details");
  const { rows: avoirs } = await pg.query(
    `SELECT id FROM public.avoirs ORDER BY id`,
  );
...
async function syncPaiements(
  ...
  const { rows } = await pg.query(`
    ...
    LEFT JOIN public.parametres mr
  ...
  for (const r of rows) {
    ...
    const modeReglement = toNullableString(r.mode_reglement);
